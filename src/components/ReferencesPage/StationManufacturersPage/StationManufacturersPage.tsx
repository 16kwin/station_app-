...
interface StationManufacturerItem {
  uid: string;
  name: string;
  description: string;
}
...
const StationManufacturersPage = () => {
  ...
  const [data, setData] = useState<StationManufacturerItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<StationManufacturerItem | null>(null);
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiStationManufacturers, { name: formName.trim(), description: formDescription.trim() });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiStationManufacturers}/${editItem.uid}`, { name: formName.trim(), description: formDescription.trim() });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      for (const uid of selectedIds) {
        await AxiosService.delete(`${ConstantInfo.restApiStationManufacturers}/${uid}`);
      }
      await fetchData();
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>Справочник: Производители станций</h1>
      </div>
            ...
            {data.map(item => {
              ...
              return (
                <div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', borderTop: '0.5px solid #E5ECF5', borderBottom: '0.5px solid #E5ECF5' }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.name)}>
                  ...
                  <span style={{ position: 'absolute', left: 85, fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 500 }}>{item.name}</span>
                </div>
              );
            })}
