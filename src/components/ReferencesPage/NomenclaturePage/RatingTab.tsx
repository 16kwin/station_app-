// RatingTab.tsx — ПОЛНЫЙ ФАЙЛ (выделение строк, удаление выделенных, кнопки button1/button4/button5)
...
const RatingTab: React.FC<CommonProps> = (props) => {
  ...
  const toggleSelect = (ratingUid: string, e: React.MouseEvent) => {
    if (e.ctrlKey || e.metaKey) {
      setSelectedIds(prev => {
        ...
        if (next.has(ratingUid)) next.delete(ratingUid);
        else next.add(ratingUid);
        return next;
      });
    } else if (e.shiftKey && lastSelectedId) {
      const allIds = ratings.map(r => r.uid);
      const lastIdx = allIds.indexOf(lastSelectedId);
      const currentIdx = allIds.indexOf(ratingUid);
      ...
      if (selectedIds.has(ratingUid) && selectedIds.size === 1) {
        setSelectedIds(new Set());
      } else {
        setSelectedIds(new Set([ratingUid]));
      }
    }
  ...
  const confirmDeleteSelected = async () => {
    try {
      ...
      console.error('Ошибка удаления отзывов:', e);
    }
  };
  ...
  const handleContextEdit = () => {
    if (!contextMenu) return;
    setEditRatingUid(contextMenu.ratingUid);
    setEditAuthor(contextMenu.author || '');
    setEditComment(contextMenu.comment || '');
    setEditRating(contextMenu.rating);
  ...
  const handleContextDelete = () => {
    ...
    Promise.all(Array.from(uidsToDelete).map(uid => 
      AxiosService.delete(ConstantInfo.restApiNomenclatureDeleteRating(uid))
    ))
      .then(() => {
        setSelectedIds(new Set());
        fetchRatings();
      })
      .catch(e => console.error('Ошибка удаления отзыва:', e));
    setContextMenu(null);
  };
                ...
                {isLoading ? (
                  ...
                  <>
                    {Array.from({ length: totalRows }).map((_, index) => {
                      ...
                      const isRealData = !!rating;
                      const isSelected = rating && selectedIds.has(rating.uid);
                      ...
                      return (
                        <div 
                          key={rating.uid} 
                          onClick={(e) => toggleSelect(rating.uid, e)}
                          onContextMenu={(e) => handleContextMenu(e, rating.uid, rating.rating, rating.comment, rating.author)}
                          style={{ 
                            height: ROW_HEIGHT, 
                            display: 'flex', 
                            alignItems: 'center', 
                        ...
                        >
                          <span style={{ position: 'absolute', left: COL_DATE, fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_NAME - COL_DATE - 20 }}>
                            {formatDate(rating.createdAt)}
                          </span>
                          
                          <div 
                            onClick={(e) => { e.stopPropagation(); handleViewComment(rating.comment, rating.author); }}
                            style={{ position: 'absolute', left: COL_NAME, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}
                          >
                          ...
                          <div style={{ position: 'absolute', left: COL_RATING }}>
                            <StarRatingSmall value={rating.rating} size={18} />
                          </div>

                          <span style={{ position: 'absolute', left: COL_AUTHOR, fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: TABLE_WIDTH - COL_AUTHOR - 40 }}>
                            {rating.author || '-'}
                          </span>
                        </div>
                      );
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowAddPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Введите текст отзыва" rows={3} style={{ width: '100%', borderRadius: 10, border: '1px solid rgba(102, 110, 254, 0.15)', padding: 12, fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', outline: 'none', boxSizing: 'border-box', backgroundColor: '#FFFFFF', resize: 'none' }} />
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  {[1, 2, 3, 4, 5].map(i => {
                    const fillPercent = Math.min(100, Math.max(0, ((hoverRating || newRating) - i + 1) * 100));
                    const isHovered = hoverRating >= i;
                    return (
                      <div key={i} onClick={() => setNewRating(i)} onMouseEnter={() => setHoverRating(i)} onMouseLeave={() => setHoverRating(0)} style={{ width: 32, height: 32, position: 'relative', cursor: 'pointer', flexShrink: 0 }}>
                        <svg width={32} height={32} viewBox="0 0 20 20" fill="none" style={{ position: 'absolute', top: 0, left: 0 }}>
                          <path d="M10 1L12.39 6.53L18.18 7.27L13.92 11.37L15.09 17.23L10 14.25L4.91 17.23L6.08 11.37L1.82 7.27L7.61 6.53L10 1Z" fill={isHovered ? '#666EFE' : '#E5E7EB'} stroke={isHovered ? '#666EFE' : '#D1D5DB'} strokeWidth="1"/>
                        </svg>
                        {!isHovered && fillPercent > 0 && (
                          <svg width={32} height={32} viewBox="0 0 20 20" fill="none" style={{ position: 'absolute', top: 0, left: 0, clipPath: `inset(0 ${100 - fillPercent}% 0 0)` }}>
                            <path d="M10 1L12.39 6.53L18.18 7.27L13.92 11.37L15.09 17.23L10 14.25L4.91 17.23L6.08 11.37L1.82 7.27L7.61 6.53L10 1Z" fill="#666EFE" stroke="#666EFE" strokeWidth="1"/>
                          </svg>
                        )}
                      </div>
                ...
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, fontWeight: 600, color: '#2D4059' }}>{hoverRating || newRating || 0}</span>
              </div>
            </div>
