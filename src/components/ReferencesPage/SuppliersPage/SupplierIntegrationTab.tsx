// SupplierIntegrationTab.tsx — ПОЛНЫЙ ФАЙЛ (в стиле IntegrationTab)
...
const SupplierIntegrationTab: React.FC<CommonSupplierProps> = (props) => {
  ...
  const toggleSelect = (itemUid: string, e: React.MouseEvent) => {
    if (e.ctrlKey || e.metaKey) {
      ...
      const lastIdx = allIds.indexOf(lastSelectedId); const currentIdx = allIds.indexOf(itemUid);
  ...
  const confirmDeleteSelected = async () => {
    try {
      for (const itemUid of selectedIds) { await AxiosService.delete(ConstantInfo.restApiSupplierDeleteIntegration(itemUid)); }
      setSelectedIds(new Set()); setShowDeleteConfirm(false); await fetchIntegrations();
  ...
  const handleContextMenu = (e: React.MouseEvent, item: IntegrationItem) => {
    ...
    if (!selectedIds.has(item.uid)) setSelectedIds(new Set([item.uid]));
    setContextMenu({ x: e.clientX, y: e.clientY, integrationUid: item.uid, exchangeType: item.exchangeType, direction: item.direction, protocol: item.protocol, targetSystem: item.targetSystem });
  };
  ...
  const handleContextEdit = () => {
    ...
    setShowEditPopup(true); setContextMenu(null);
  };

  const handleContextDelete = () => {
    if (!contextMenu) return;
    if (!confirm('Удалить запись интеграции?')) { setContextMenu(null); return; }
    const uidsToDelete = selectedIds.has(contextMenu.integrationUid) ? selectedIds : new Set([contextMenu.integrationUid]);
    Promise.all(Array.from(uidsToDelete).map(uid => AxiosService.delete(ConstantInfo.restApiSupplierDeleteIntegration(uid))))
      .then(() => { setSelectedIds(new Set()); fetchIntegrations(); }).catch(e => console.error(e));
    setContextMenu(null);
  };
  ...
  const handleEditSubmit = async () => {
    ...
    setIsEditing(true);
    try {
      await AxiosService.delete(ConstantInfo.restApiSupplierDeleteIntegration(editIntegrationUid));
      await AxiosService.post(ConstantInfo.restApiSupplierIntegrations(uid), { exchangeType: editExchangeType, direction: editDirection, protocol: editProtocol, targetSystem: editTargetSystem });
      await fetchIntegrations(); setShowEditPopup(false);
  ...
  const getRowSeparator = (index: number, isRealData: boolean): React.CSSProperties => {
    ...
    const isFirst = index === 0; const isLast = index === integrations.length - 1;
  ...
  return (
    <div style={cs}>
                ...
                {isLoading ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#9CA3AF' }}>Загрузка...</span></div> : (
                  <>
                    {Array.from({ length: totalRows }).map((_, index) => {
                      const item = integrations[index]; const isRealData = !!item; const isSelected = item && selectedIds.has(item.uid);
                      ...
                      return (
                        <div key={item.uid} onClick={(e) => toggleSelect(item.uid, e)} onContextMenu={(e) => handleContextMenu(e, item)} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#DEEEFF' : '#FFFFFF', position: 'relative', boxSizing: 'border-box', cursor: 'pointer', userSelect: 'none', ...getRowSeparator(index, true) }}>
      ...
      {showAddPopup && (<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowAddPopup(false)}><div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}><h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Добавление интеграции</h3><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Тип обмена</label><select value={newExchangeType} onChange={e => setNewExchangeType(e.target.value)} style={selectStyle}>{EXCHANGE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Направление</label><select value={newDirection} onChange={e => setNewDirection(e.target.value)} style={selectStyle}>{DIRECTIONS.map(d => <option key={d} value={d}>{d}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Протокол</label><select value={newProtocol} onChange={e => setNewProtocol(e.target.value)} style={selectStyle}>{PROTOCOLS.map(p => <option key={p} value={p}>{p}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Обмен с системой</label><select value={newTargetSystem} onChange={e => setNewTargetSystem(e.target.value)} style={selectStyle}>{TARGET_SYSTEMS.map(s => <option key={s} value={s}>{s}</option>)}</select></div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={handleAddSubmit} disabled={isAdding} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: 'none', backgroundColor: !isAdding ? '#666EFE' : '#BCC8FF', cursor: !isAdding ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#FFFFFF' }}>{isAdding ? 'Добавление...' : 'Добавить'}</button><button onClick={() => setShowAddPopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Отмена</button></div></div></div>)}

      {showEditPopup && (<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}><div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}><h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование интеграции</h3><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Тип обмена</label><select value={editExchangeType} onChange={e => setEditExchangeType(e.target.value)} style={selectStyle}>{EXCHANGE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Направление</label><select value={editDirection} onChange={e => setEditDirection(e.target.value)} style={selectStyle}>{DIRECTIONS.map(d => <option key={d} value={d}>{d}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Протокол</label><select value={editProtocol} onChange={e => setEditProtocol(e.target.value)} style={selectStyle}>{PROTOCOLS.map(p => <option key={p} value={p}>{p}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Обмен с системой</label><select value={editTargetSystem} onChange={e => setEditTargetSystem(e.target.value)} style={selectStyle}>{TARGET_SYSTEMS.map(s => <option key={s} value={s}>{s}</option>)}</select></div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={handleEditSubmit} disabled={isEditing} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: 'none', backgroundColor: !isEditing ? '#666EFE' : '#BCC8FF', cursor: !isEditing ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#FFFFFF' }}>{isEditing ? 'Сохранение...' : 'Сохранить'}</button><button onClick={() => setShowEditPopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Отмена</button></div></div></div>)}
