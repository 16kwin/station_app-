// NomenclatureTypesPage.tsx — ПОЛНЫЙ ФАЙЛ (единый стиль)
...
interface TypeProductItem {
  uid: string;
  typeName: string;
  typePurposeUid: string;
  typePurposeName: string;
  typeMaterialName: string;
}

interface TypePurposeOption { uid: string; typeName: string; }
interface TypeMaterialOption { uid: string; typeName: string; }

const NomenclatureTypesPage = () => {
  const { activeTabId } = useTabs();
  const tabIdRef = useRef<string | null>(null);
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [hasVerticalScroll, setHasVerticalScroll] = useState(false);
  const [hasHorizontalScroll, setHasHorizontalScroll] = useState(false);
  const [data, setData] = useState<TypeProductItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<TypeProductItem | null>(null);
  const [formName, setFormName] = useState('');
  const [formTypeMaterialUid, setFormTypeMaterialUid] = useState('');
  const [formTypePurposeUid, setFormTypePurposeUid] = useState('');
  ...
  const [allTypePurposes, setAllTypePurposes] = useState<TypePurposeOption[]>([]);
  const [filteredTypePurposes, setFilteredTypePurposes] = useState<TypePurposeOption[]>([]);
  ...
  const COL_GROUP = 700;
  const COL_ACCOUNT = 1000;

  useEffect(() => { tabIdRef.current = activeTabId; }, []);

  const fetchData = async () => {
    try { const r = await AxiosService.get(ConstantInfo.restApiNomenclatureTypeProducts); setData(r.data || []); }
    catch (e) { console.error(e); } finally { setIsLoading(false); }
  };

  const loadReferences = async () => {
    try {
      const [mR, pR] = await Promise.all([AxiosService.get(ConstantInfo.restApiNomenclatureTypeMaterials), AxiosService.get(ConstantInfo.restApiNomenclatureTypePurposes)]);
      setTypeMaterials(mR.data || []); setAllTypePurposes(pR.data || []);
  ...
  const handleTypeMaterialChange = (uid: string) => {
    setFormTypeMaterialUid(uid); setFormTypePurposeUid('');
    if (uid) setFilteredTypePurposes(allTypePurposes.filter(p => (p as any).typeMaterialUid === uid));
    else setFilteredTypePurposes([]);
  };
  ...
  const handleCreateClick = () => { setFormName(''); setFormTypeMaterialUid(''); setFormTypePurposeUid(''); setFilteredTypePurposes([]); setEditItem(null); setShowCreatePopup(true); };
  const handleCreateSubmit = async () => { if (!formName.trim()) return; setIsSaving(true); try { await AxiosService.post(ConstantInfo.restApiNomenclatureTypeProducts, { name: formName.trim(), typePurposeUid: formTypePurposeUid || null }); await fetchData(); setShowCreatePopup(false); } catch (e) { console.error(e); } finally { setIsSaving(false); } };
  const handleEditClick = () => { if (!contextMenu) return; const item = data.find(d => d.uid === contextMenu.uid); if (item) { setEditItem(item); setFormName(item.typeName); const purpose = allTypePurposes.find(p => p.uid === item.typePurposeUid); const muid = purpose ? (purpose as any).typeMaterialUid || '' : ''; setFormTypeMaterialUid(muid); setFormTypePurposeUid(item.typePurposeUid || ''); if (muid) setFilteredTypePurposes(allTypePurposes.filter(p => (p as any).typeMaterialUid === muid)); else setFilteredTypePurposes([]); setShowEditPopup(true); } setContextMenu(null); };
  const handleEditSubmit = async () => { if (!editItem || !formName.trim()) return; setIsSaving(true); try { await AxiosService.patch(`${ConstantInfo.restApiNomenclatureTypeProducts}/${editItem.uid}`, { name: formName.trim(), typePurposeUid: formTypePurposeUid || null }); await fetchData(); setShowEditPopup(false); setEditItem(null); } catch (e) { console.error(e); } finally { setIsSaving(false); } };
  const handleDeleteClick = () => { if (selectedIds.size === 0) return; setShowDeleteConfirm(true); };
  const confirmDelete = async () => { try { for (const uid of selectedIds) { await AxiosService.delete(`${ConstantInfo.restApiNomenclatureTypeProducts}/${uid}`); } await fetchData(); setSelectedIds(new Set()); setShowDeleteConfirm(false); } catch (e) { console.error(e); } };
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}><h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>Справочник: Виды номенклатуры</h1></div>
      ...
      <div style={{ position: 'absolute', top: 154, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={{ height: HEADER_HEIGHT, minHeight: HEADER_HEIGHT, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, position: 'relative' }}><EmptySquare isSelected={isAllSelected} onClick={toggleSelectAll} /><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 600, color: '#FFFFFF', marginLeft: 47 }}>НАИМЕНОВАНИЕ</span><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 600, color: '#FFFFFF', position: 'absolute', left: COL_GROUP }}>ГРУППА НОМ.</span><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 600, color: '#FFFFFF', position: 'absolute', left: COL_ACCOUNT }}>ГРУППА УЧЕТА</span></div>
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => { const isSelected = selectedIds.has(item.uid); return (<div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', borderTop: '0.5px solid #E5ECF5', borderBottom: '0.5px solid #E5ECF5' }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.typeName)}><div style={{ paddingLeft: 20, display: 'flex', alignItems: 'center' }}><EmptySquare isSelected={isSelected} onClick={() => toggleSelectItem(item.uid)} /></div><img src={Popup3} alt="" style={{ width: 20, height: 20, flexShrink: 0, marginLeft: 19 }} /><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059', marginLeft: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_GROUP - 120 }}>{item.typeName}</span><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: COL_GROUP, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_ACCOUNT - COL_GROUP - 20 }}>{item.typePurposeName || '—'}</span><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: COL_ACCOUNT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: TABLE_WIDTH - COL_ACCOUNT - 60 }}>{item.typeMaterialName || '—'}</span></div>); })}
      ...
      {showCreatePopup && (<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}><div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}><h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание вида номенклатуры</h3><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Название</label><input type="text" value={formName} onChange={e => setFormName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateSubmit(); else if (e.key === 'Escape') setShowCreatePopup(false); }} placeholder="Введите название" autoFocus style={inputStyle} /></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа учета</label><select value={formTypeMaterialUid} onChange={e => handleTypeMaterialChange(e.target.value)} style={selectStyle}><option value="">Выберите группу учета</option>{typeMaterials.map(tm => <option key={tm.uid} value={tm.uid}>{tm.typeName}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа номенклатуры</label><select value={formTypePurposeUid} onChange={e => setFormTypePurposeUid(e.target.value)} style={{ ...selectStyle, opacity: formTypeMaterialUid ? 1 : 0.5, cursor: formTypeMaterialUid ? 'pointer' : 'not-allowed' }} disabled={!formTypeMaterialUid}><option value="">{formTypeMaterialUid ? 'Выберите группу' : 'Сначала выберите группу учета'}</option>{filteredTypePurposes.map(tp => <option key={tp.uid} value={tp.uid}>{tp.typeName}</option>)}</select></div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => setShowCreatePopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Отмена</button><button onClick={handleCreateSubmit} disabled={isSaving || !formName.trim()} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: 'none', backgroundColor: formName.trim() && !isSaving ? '#666EFE' : '#BCC8FF', cursor: formName.trim() && !isSaving ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#FFFFFF' }}>{isSaving ? 'Сохранение...' : 'Создать'}</button></div></div></div>)}
      {showEditPopup && (<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}><div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}><h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование вида номенклатуры</h3><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Название</label><input type="text" value={formName} onChange={e => setFormName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleEditSubmit(); else if (e.key === 'Escape') setShowEditPopup(false); }} placeholder="Введите название" autoFocus style={inputStyle} /></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа учета</label><select value={formTypeMaterialUid} onChange={e => handleTypeMaterialChange(e.target.value)} style={selectStyle}><option value="">Выберите группу учета</option>{typeMaterials.map(tm => <option key={tm.uid} value={tm.uid}>{tm.typeName}</option>)}</select></div><div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа номенклатуры</label><select value={formTypePurposeUid} onChange={e => setFormTypePurposeUid(e.target.value)} style={{ ...selectStyle, opacity: formTypeMaterialUid ? 1 : 0.5, cursor: formTypeMaterialUid ? 'pointer' : 'not-allowed' }} disabled={!formTypeMaterialUid}><option value="">{formTypeMaterialUid ? 'Выберите группу' : 'Сначала выберите группу учета'}</option>{filteredTypePurposes.map(tp => <option key={tp.uid} value={tp.uid}>{tp.typeName}</option>)}</select></div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => setShowEditPopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Отмена</button><button onClick={handleEditSubmit} disabled={isSaving || !formName.trim()} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: 'none', backgroundColor: formName.trim() && !isSaving ? '#666EFE' : '#BCC8FF', cursor: formName.trim() && !isSaving ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#FFFFFF' }}>{isSaving ? 'Сохранение...' : 'Сохранить'}</button></div></div></div>)}
...
export default NomenclatureTypesPage;