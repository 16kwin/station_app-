// StationModelsPage.tsx — ПОЛНЫЙ ФАЙЛ
...
const StationModelsPage = () => {
  ...
  const [data, setData] = useState<StationModelItem[]>([]);
  ...
  const COL_NAME = 170;
  const COL_TYPE = 470;
  const COL_MANUFACTURER = 640;
  const COL_ARTICLE = 840;
  const COL_PURPOSE = 1000;
  const COL_REVISION = 1350;
  ...
  const handleCreateClick = async () => {
    try {
      ...
      openTab(`/references/station-models/create/${newUid}`, `Модель станции: ${String(code).padStart(4, '0')}`, null);
    } catch (error) {
      console.error('Ошибка генерации кода:', error);
      const newUid = crypto.randomUUID();
      openTab(`/references/station-models/create/${newUid}`, 'Модель станции (новый)', null);
    }
  };
  ...
  const cellStyle = (left: number, maxWidth: number): React.CSSProperties => ({
    position: 'absolute', left, fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400,
    color: '#2D4059', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth
  });
  const cellStyleGray = (left: number, maxWidth: number): React.CSSProperties => ({
    position: 'absolute', left, fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400,
    color: '#6B7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth
  });
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>Справочник: Модели станций</h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 154, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={{ height: HEADER_HEIGHT, minHeight: HEADER_HEIGHT, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, position: 'relative' }}>
            ...
            <span style={{ ...headerStyle, left: COL_NAME }}>МОДЕЛЬ</span>
            <span style={{ ...headerStyle, left: COL_TYPE }}>ТИП</span>
            <span style={{ ...headerStyle, left: COL_MANUFACTURER }}>ПРОИЗВОДИТЕЛЬ</span>
            <span style={{ ...headerStyle, left: COL_ARTICLE }}>АРТИКУЛ</span>
            <span style={{ ...headerStyle, left: COL_PURPOSE }}>НАЗНАЧЕНИЕ</span>
            <span style={{ ...headerStyle, left: COL_REVISION }}>РЕВИЗИЯ</span>
          </div>
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => {
              ...
              return (
                <div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', borderTop: '0.5px solid #E5ECF5', borderBottom: '0.5px solid #E5ECF5' }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.name)} onDoubleClick={() => openTab(`/references/station-models/edit/${item.uid}`, `Модель: ${item.name}`, null)}>
                  ...
                  <span style={cellStyleGray(COL_TYPE, COL_MANUFACTURER - COL_TYPE - 20)}>{item.typeName || '-'}</span>
                  <span style={cellStyleGray(COL_MANUFACTURER, COL_ARTICLE - COL_MANUFACTURER - 20)}>{item.manufacturerName || '-'}</span>
                  <span style={cellStyleGray(COL_ARTICLE, COL_PURPOSE - COL_ARTICLE - 20)}>{item.article || '-'}</span>
                  <span style={cellStyleGray(COL_PURPOSE, COL_REVISION - COL_PURPOSE - 20)}>{item.purpose || '-'}</span>
                  <span style={cellStyleGray(COL_REVISION, 300)}>{item.revision || '-'}</span>
                </div>
              );
            })}
