// UnitsPage.tsx — ПОЛНЫЙ ФАЙЛ (единый стиль)
...
const UnitsPage = () => {
  ...
  const fetchData = async () => {
    try { const r = await AxiosService.get(ConstantInfo.restApiNomenclatureMeasures); setData(r.data || []); }
  ...
  const handleCreateSubmit = async () => { if (!formName.trim()) return; setIsSaving(true); try { await AxiosService.post(ConstantInfo.restApiNomenclatureMeasures, { name: formName.trim(), description: formDescription.trim() }); await fetchData(); setShowCreatePopup(false); } catch (e) { console.error(e); } finally { setIsSaving(false); } };
  ...
  const handleEditSubmit = async () => { if (!editItem || !formName.trim()) return; setIsSaving(true); try { await AxiosService.patch(`${ConstantInfo.restApiNomenclatureMeasures}/${editItem.uid}`, { name: formName.trim(), description: formDescription.trim() }); await fetchData(); setShowEditPopup(false); setEditItem(null); } catch (e) { console.error(e); } finally { setIsSaving(false); } };
  const handleDeleteClick = () => { if (selectedIds.size === 0) return; setShowDeleteConfirm(true); };
  const confirmDelete = async () => { try { for (const uid of selectedIds) { await AxiosService.delete(`${ConstantInfo.restApiNomenclatureMeasures}/${uid}`); } await fetchData(); setSelectedIds(new Set()); setShowDeleteConfirm(false); } catch (e) { console.error(e); } };
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}><h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>Справочник: Единицы измерения</h1></div>
      ...
      <div style={{ position: 'absolute', top: 154, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => { const isSelected = selectedIds.has(item.uid); return (<div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', borderTop: '0.5px solid #E5ECF5', borderBottom: '0.5px solid #E5ECF5' }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.name)}><div style={{ paddingLeft: 20, display: 'flex', alignItems: 'center' }}><EmptySquare isSelected={isSelected} onClick={() => toggleSelectItem(item.uid)} /></div><img src={Popup5} alt="" style={{ width: 20, height: 20, flexShrink: 0, marginLeft: 19 }} /><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059', marginLeft: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_DESCRIPTION - 120 }}>{item.name}</span><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: COL_DESCRIPTION, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: TABLE_WIDTH - COL_DESCRIPTION - 60 }}>{item.description || '—'}</span></div>); })}
